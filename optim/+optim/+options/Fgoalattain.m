classdef (Sealed) Fgoalattain < optim.options.SingleAlgorithm
%

%Fgoalattain Options for FGOALATTAIN
%
%   The OPTIM.OPTIONS.FGOALATTAIN class allows the user to create a set of
%   options for the FGOALATTAIN solver. For a list of options that can be
%   set, see the documentation for FGOALATTAIN.
%
%   OPTS = OPTIM.OPTIONS.FGOALATTAIN creates a set of options for
%   FGOALATTAIN with the options set to their default values.
%
%   OPTS = OPTIM.OPTIONS.FGOALATTAIN(PARAM, VAL, ...) creates a set of
%   options for FGOALATTAIN with the named parameters altered with the
%   specified values.
%
%   OPTS = OPTIM.OPTIONS.FGOALATTAIN(OLDOPTS, PARAM, VAL, ...) creates a
...
%
%   For more information, type "doc fgoalattain" and see the "Options"
%   section in the FGOALATTAIN documentation page.
        ConstraintTolerance
...
%EQUALITYGOALCOUNT Number of objectives for which it is required for the
%                  objective fun to equal the goal
%             
%   For more information, type "doc fgoalattain" and see the "Options"
%   section in the FGOALATTAIN documentation page.
        EqualityGoalCount
...
%                    measure
%
%   For more information, type "doc fgoalattain" and see the "Options" section
%   in the FGOALATTAIN documentation page.
        OptimalityTolerance               
...
%FINDIFFTYPE Finite differences type
        FinDiffType
        
%FUNVALCHECK Check validity of user function evaluations
        FunValCheck
        
%GOALSEXACTACHIEVE Number of objectives for which it is required for the
%                  objective fun to equal the goal
        GoalsExactAchieve
...
% DERIVATIVECHECK Removed from fgoalattain; this is to preserve backward
% compatibility
%
        DerivativeCheck
    end
...
% Properties
    ...
    methods (Hidden)
        ...
        function obj = Fgoalattain(varargin)
%Fgoalattain Options for FGOALATTAIN
%
%   The OPTIM.OPTIONS.FGOALATTAIN class allows the user to create a set of
%   options for the FGOALATTAIN solver. For a list of options that can be
...
%   See also OPTIM.OPTIONS.SINGLEALGORITHM, OPTIM.OPTIONS.SOLVEROPTIONS
            ...
            % FgoalattainVersion instead.
            obj.Version = 1;
            
            obj.FgoalattainVersion = 2;
...
%   its 'default' value or has been 'selected'. 
    ...
    methods
        ...
        function obj = set.EqualityGoalCount(obj, value)
            obj = setAliasProperty(obj, 'EqualityGoalCount', 'GoalsExactAchieve', value);
        end
        ...
        function obj = set.GoalsExactAchieve(obj, value)
            obj = setProperty(obj, 'GoalsExactAchieve', value);
        end
        ...
        function value = get.EqualityGoalCount(obj)
            value = obj.OptionsStore.Options.GoalsExactAchieve;
        end        
        ...
        function value = get.GoalsExactAchieve(obj)
            value = obj.OptionsStore.Options.GoalsExactAchieve;
        end
    ...
    methods (Static = true)
        function obj = loadobj(obj)
            ...
            if isstruct(obj) && obj.Version == 1
                ...
                % correctly.
                obj.FgoalattainVersion = [];
                
            end
            
            % Add TolFunValue
            if isempty(obj.FgoalattainVersion) || obj.FgoalattainVersion < 2
                % Add TolFunValue
...
%   the full OptionsStore. See below for an example for Fgoalattain.
...
OS.Defaults.GoalsExactAchieve = 0;
...
% Helper function to generate constant property metadata for the
% Fgoalattain
% options class.
...
propInfo.EqualityGoalCount = TypeInfo.integerType();
