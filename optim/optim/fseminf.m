...
%       x = fseminf(@myfun,x0,2,@myseminfcon)
%
...
%       K2 = 4*x*sin(w2) - 1;
%
%   To run this example for 0 <= x <= 2:
%
%     lb = 0;
%     ub = 2;
%     x0 = 1/10;
%     x = fseminf(@myfun,x0,2,@myseminfcon,[],[],[],[],lb,ub)
%
%   See also OPTIMOPTIONS, @, FGOALATTAIN, LSQNONLIN.
...
[options, optionFeedback] = prepareOptionsForSolver(options, 'fseminf');
...
msg = isoptimargdbl('FSEMINF', {'X0','NTHETA','A','B','Aeq','Beq','LB','UB'}, ...
                                 x,   ntheta,  A,  B,  Aeq,  Beq,  LB,  UB);
if ~isempty(msg)
    error('optim:fseminf:NonDoubleInput',msg);
end
...
flags.hess = strcmp(optimget(options,'Hessian',defaultopt,'fast',allDefaultOpts),'on');
if flags.hess
...
flags.gradconst = strcmp(optimget(options,'GradConstr',defaultopt,'fast',allDefaultOpts),'on');
...
if numel(initVals.g) ~= sizes.nVar
   error('optim:fseminf:InvalidSizeOfGradient', ...
...
if ~isempty( isoptimargdbl('FSEMINF', {'f','g','c','ceq'}, ...
                    initVals.f, initVals.g, initVals.ncineq, initVals.nceq) )
    error('optim:fseminf:NonDoubleFunVal',getString(message('optimlib:commonMsgs:NonDoubleFunVal','FSEMINF')));
end
...
[x,FVAL,LAMBDA,EXITFLAG,OUTPUT]=...
   ...
   finDiffFlags,verbosity,flags,initVals,problemInfo,optionFeedback, ...
   ntheta,userconfcn,varargin{:});
