...
% Another special code to unset Hessian option if sqp/active-set
% algorithms are used. These algorithms do not use Hessian and warn
% when it is set to 'on'.
if strcmpi(probStruct.solver, 'fmincon') && ~isempty(optStruct.Hessian) && ...
        any(strcmpi(optStruct.Algorithm, {'sqp','active-set'}))
        optStruct.Hessian = [];
end
...
stringOnly =   {'NodeSearchStrategy','MeritFunction', ...
    'LargeScale','Jacobian', 'HessUpdate', ...
