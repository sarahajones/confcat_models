...
if ~errCheck         
    ...
    if ~isempty( isoptimargdbl('GOALCON', {'f','gf','c','ceq','gc','gceq'}, ...
                                            f,  gf,  c,  ceq,  gc,  gceq) )
        [stack,~] = dbstack;
        [~,caller] = fileparts(stack(2).file);
        caller = upper(caller);
        error('optim:goalcon:NonDoubleFunVal',getString(message('optimlib:commonMsgs:NonDoubleFunVal',caller)));
    end
