...
defaultVariableNames = {'optimprob'; ' '; 'options'; 'optimresults'};
...
function OKCallback(obj, eventdata, dialog, cb, e, optStruct, probStruct,resultStruct)
    ...
    if length(varnames) == length(uniqueArray)
        if get(cb{CB_PROBLEM}, 'Value') == 1  % Export problem structure
            ...
            options = createOptionsStruct(probStruct.solver,optStruct);
            probStruct.options = optimset2optimoptions(probStruct.solver,options);
            if includeRestart
        ...
        if get(cb{CB_OPTION}, 'Value') == 1   % Export options structure
            ...
            optStruct = optimset2optimoptions(probStruct.solver,optStruct);
            if includeRestart
