...
for i = 1:length(whoslist)
    if strcmp(whoslist(i).class, 'struct') && strcmp(num2str(whoslist(i).size), '1  1')
        ...
        if validOptimProblemStruct(s,requiredFields,validValues) && ...
            ...
            var = evalin('base',whoslist(i).name);
            namestr{end+1} = [whoslist(i).name, ' - ''',char(var.solver),''' problem'];
...
if isempty(names) 
    ...
    [selectionIndex, Answer] = listdlg('ListString', namestr, ...
    ...
    if Answer == 1
        ...
        probStruct.options = optimoptions2optimset(probStruct.options);
